...
async def create_tables():
    ...
    async with pool.acquire() as c:
        ...
        CREATE TABLE IF NOT EXISTS sms_codes (
            ...
            purpose         VARCHAR(20) DEFAULT 'register'
                            CHECK (purpose IN ('register','login','reset','reset_attempt')),
    ...
    other_migrations = [
        ...
        # Второй телефон (запасной, на этот конкретный лид/заказ — НЕ contacts.phone2)
        # и адрес доставки, если отличается от адреса вывоза (short_address/address).
        # Пусто = доставка на тот же адрес. См. запрос пользователя 2026-09-17.
        ...
        "ALTER TABLE orders      ADD COLUMN IF NOT EXISTS delivery_location         TEXT DEFAULT ''",
        ...
        "ALTER TABLE leads       ADD COLUMN IF NOT EXISTS delivery_location         TEXT DEFAULT ''",
        ...
        "ALTER TABLE staff       ADD COLUMN IF NOT EXISTS can_edit_price       BOOLEAN DEFAULT FALSE",
        ...
        # убрать кнопку "Взять лид" и дописать "✅ Взял(а): Имя", как это уже
        # происходит при нажатии кнопки внутри Telegram.
        ...
        # ── Чёрный список: не участвовать в автодозвоне/SMS. Причина хранится
        # только в blacklist (единый источник) — в остальных таблицах лёгкий флаг.
        ...
        # при каждой попытке ввести код сброса пароля.
    ...
    async with pool.acquire() as c:
        ...
        -- Найдено 2026-08-21 в SaaS-форке (тот же баг здесь) — при общем устройстве
        -- (например один компьютер на несколько сотрудников) второй "крал" push-
        -- подписку у первого. Теперь UNIQUE на пару (staff_id, endpoint) — один
        -- браузер может держать подписки сразу нескольких staff_id одновременно.
...
async def get_all_site_users(search: str = "", limit: int = 500):
    ...
    base = """
        SELECT u.id, u.phone, u.first_name, u.is_verified, u.tg_id,
               u.address, u.car_plate, u.osago_expiry, u.blacklisted,
...
async def get_order_items_billing(order_id: int) -> dict:
    ...
    for it in items:
        ...
        # реальных денежных расчётов: если менеджер скорректировал замер
        # (actual_sqm/actual_total_sum), клампинг и итог считаются ПО СКОРРЕКТИРОВАННЫМ
        # значениям, а не по изначально сданным sqm/total_sum.
        price = float(it["price_per_sqm"] or 0)
    ...
    for gkey, g in groups.items():
        ...
        group_list.append({
            ...
            "unit_key": (catalog or {}).get("unit_key") or "m2", "price": price,
        })
...
async def get_admin_orders(status: str = None, statuses: list = None, branch: str = None,
    ...
    search_phone=False — не искать по client_phone: для сотрудников с hide_client_phone,
    иначе поиск по номеру работал бы оракулом (подтвердил бы, что номер есть в базе, и
    заодно показал бы ФИО/адрес клиента), хотя сам номер в ответе замаскирован.
    ...
    async with pool.acquire() as conn:
        ...
        if search:
            idx = len(params) + 1
            phone_clause = f"OR o.client_phone ILIKE ${idx}" if search_phone else ""
            conditions.append(f"""(
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO leads (client_name, client_phone, client_phone2, service, branch,
                               city, address, short_address, delivery_address, delivery_short_address,
        ...
        """, data.get("client_name"), data["client_phone"], data.get("client_phone2"),
            ...
            data.get("address"), data.get("short_address", ""),
...
# ══════════════════════════════════════════════════════════════════════════════
# ЧЁРНЫЙ СПИСОК (не участвуют в автодозвоне/SMS) — причина хранится только
...
async def _mark_blacklisted_everywhere(conn, phone: str, value: bool):
    for table in _BLACKLIST_SOURCE_TABLES:
        try:
            await conn.execute(f"UPDATE {table} SET blacklisted=$1 WHERE phone=$2", value, phone)
        except Exception:
...
async def remove_from_blacklist(phone: str) -> bool:
    if not pool or not phone:
        return False
    async with pool.acquire() as conn:
        res = await conn.execute("DELETE FROM blacklist WHERE phone=$1", phone)
        await _mark_blacklisted_everywhere(conn, phone, False)
        return res == "DELETE 1"
...
async def get_blacklist_list(search: str = "", limit: int = 50, offset: int = 0) -> list[dict]:
    ...
    async with pool.acquire() as conn:
        if search:
            rows = await conn.fetch(
                "SELECT * FROM blacklist WHERE phone ILIKE $1 OR note ILIKE $1 "
...
async def get_blacklist_count(search: str = "") -> int:
    ...
    async with pool.acquire() as conn:
        if search:
            return await conn.fetchval(
                "SELECT COUNT(*) FROM blacklist WHERE phone ILIKE $1 OR note ILIKE $1",
                f"%{search}%")
        return await conn.fetchval("SELECT COUNT(*) FROM blacklist")

async def is_phone_blacklisted(phone: str) -> bool:
    if not pool or not phone:
        return False
    async with pool.acquire() as conn:
        return bool(await conn.fetchval("SELECT 1 FROM blacklist WHERE phone=$1", phone))

async def sync_blacklist_flags() -> dict:
    """Кнопка «Обновить» на странице ЧС: досвежа проставляет флаг blacklisted
    во всех 4 таблицах по текущему списку blacklist (для контактов/пользователей,
    добавленных ПОСЛЕ того как номер уже был в ЧС), и снимает флаг там, где
    номера больше нет в blacklist."""
    if not pool:
        return {}
    stats = {}
    async with pool.acquire() as conn:
        for table in _BLACKLIST_SOURCE_TABLES:
            try:
                r1 = await conn.execute(f"""
                    ...
                    WHERE phone IN (SELECT phone FROM blacklist) AND blacklisted IS DISTINCT FROM TRUE
                """)
                r2 = await conn.execute(f"""
                    UPDATE {table} SET blacklisted=FALSE
                    WHERE phone NOT IN (SELECT phone FROM blacklist) AND blacklisted=TRUE
                """)
                stats[table] = {"marked": int(r1.split()[-1]), "cleared": int(r2.split()[-1])}
            except Exception:
                stats[table] = {"marked": 0, "cleared": 0}  # таблица не существует
    return stats
...
async def bulk_insert_contacts(rows: list[dict]) -> dict:
    """Массовая вставка контактов. Возвращает {ok, dup, err, blacklisted}."""
    if not pool:
        return {"ok": 0, "dup": 0, "err": len(rows), "blacklisted": 0}
    ok = dup = err = blacklisted = 0
    async with pool.acquire() as conn:
        bl_phones = {r["phone"] for r in await conn.fetch("SELECT phone FROM blacklist")}
        for r in rows:
    ...
    async with pool.acquire() as conn:
        if status == 'washing':
            ...
            sql = """UPDATE orders SET status=$2, postponed_until=NULL,
                     postpone_reason=NULL, postponed_by=NULL WHERE id=$1 RETURNING *"""
        else:
...
async def set_order_postpone(order_id: int, until_date, reason: str, staff_id: int) -> dict:
    ...
    async with pool.acquire() as conn:
        ...
        """, order_id, until_date, reason, staff_id)
        return dict(row) if row else {}

async def get_orders_for_postpone_reminder() -> list:
    """Заказы, у которых отсрочка доставки истекает завтра — для ежедневного
    напоминания администраторам/менеджерам созвониться с клиентом."""
    if not pool: return []
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
...
async def get_order_managers(branch: str | None) -> list:
    """admin (видят все филиалы) + manager этого филиала, с привязанным TG."""
    if not pool: return []
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT id, first_name, last_name, tg_id FROM staff
            WHERE active=TRUE AND tg_id IS NOT NULL
              AND (role='admin' OR (role='manager' AND (branch=$1 OR branch IS NULL)))
        """, branch)
        return [dict(r) for r in rows]
...
async def upsert_push_subscription(staff_id: int, endpoint: str, p256dh: str, auth: str):
    ...
    # endpoint) позволял НЕСКОЛЬКИМ staff_id держать подписку на один и тот же
    # endpoint одновременно (см. историю в CREATE UNIQUE INDEX idx_push_staff_endpoint
    # чуть выше), но т.к. initWebPush() на фронте переиспользует уже существующую
    # PushManager-подписку браузера (getSubscription() перед subscribe()), это
    # приводило к тому, что push, адресованный ОДНОМУ сотруднику, физически
    # доставлялся на общее устройство и был виден любому, кто ранее подписывался
    # с него — например уведомление "Замер утверждён" мойщику показывалось и
    # менеджеру, если тот раньше логинился на том же устройстве (ARTEZ-1226,
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT r.branch, r.tg_delivery_msg_ids, ro.sort_order,
                   ...
                   o.address, o.short_address, o.delivery_address, o.delivery_short_address,
                   o.location, o.location_address,
                   o.delivery_location, o.delivery_location_address,
        ...
        rows = await conn.fetch("""
            SELECT o.id, o.order_num, o.status, o.debt_due_date, o.debt_approved_at,
                   ...
                   GREATEST(0,
                     ...
                     -- входит в долг) — та же формула, что в staff.html/admin.html (_orderNet)
                     -- и в /my-route/.../close-with-debt.
