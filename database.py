...
async def upsert_push_subscription(staff_id: int, endpoint: str, p256dh: str, auth: str):
    ...
    # физический браузер, использованный для входа под разными staff_id (общий
    # компьютер на несколько сотрудников), "крал" бы подписку друг у друга при
    # переподписке (см. комментарий у CREATE UNIQUE INDEX idx_push_staff_endpoint).
    if not pool: return
    async with pool.acquire() as conn:
        await conn.execute("""
