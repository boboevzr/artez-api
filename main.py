...
async def _send_postpone_reminders():
    ...
    напомнить admin/менеджерам филиала созвониться с клиентом."""
    orders = await db.get_orders_for_postpone_reminder()
    if not orders:
        return
    for o in orders:
        managers = await db.get_order_managers(o.get("branch"))
        if not managers:
            continue
...
async def send_route_to_delivery_group(route_id: int, me=Depends(get_current_staff)):
    ...
    # Удаляем предыдущие сообщения канала (старые записи "__group__" из уведомления
    # в группу, которого больше нет, — оставляем как есть, само потеряется из вида)
...
async def create_lead(req: LeadCreateRequest, staff=Depends(get_current_staff)):
    ...
    if lead:
        ...
        # в единую базу контактов вообще — закрываем дыру.
...
# ── Единая база "актуальных" контактов (реальные клиенты + дозвонившиеся из
...
async def active_contacts_list(search: str = "", limit: int = 50, offset: int = 0,
    ...
    count = await db.get_active_contacts_count()
...
async def active_contacts_backfill(_=Depends(_get_admin)):
    ...
    пользователи сайта) в единую базу актуальных контактов."""
...
async def update_order_data(order_id: int, body: dict = Body(...), staff=Depends(get_current_staff)):
    ...
    # не колонка БД — get_current_staff() возвращает dict(staff), где такого ключа нет).
    # Из-за этого сотрудник с role="admin" ошибочно попадал под то же ограничение
    # по статусу, что и остальные роли — не мог редактировать заказ в "Доставлено".
    if (role != "admin"
...
async def admin_update_order_item(order_id: int, item_id: int,
    ...
    # и сотрудников с отдельным правом can_edit_price (см. запрос пользователя 2026-09-16:
    # менеджеры не замечали примечание о скидке клиенту, а на "Доставке" исправить
    # цену было нельзя даже им — приходилось откатывать статус на "Мойку").
    order = await db.get_order_by_id(order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Заказ не найден")
...
async def create_service_region_ep(
    ...
    poi_type:          str  = Body(None,  embed=True),
    staff=Depends(get_current_staff),
):
    if staff.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Только для admin")
    region = await db.create_service_region(
        ...
        lat, location_address, sort_order, note, name_ru_full, name_uz_full, not_exists, polygon, poi_type)
...
async def update_service_region_ep(
    ...
    fields = {k: v for k, v in {
        ...
        "note": note, "not_exists": not_exists, "poi_type": poi_type,
...
def _sniff_media_type(content: bytes, file_path: str = "") -> str:
    ...
    файл без расширения (обнаружено на живых чеках Перевод/Карта, 2026-09-05)."""
        ...
        row = await conn.fetchrow(
            """UPDATE staff
               SET can_edit_items=$2, can_measure=$3, can_approve_measure=$4,
                   ...
                   hide_client_phone=$18, can_edit_price=$19
               WHERE id=$1
            ...
            can_drive, can_view_timesheet, hide_client_phone, can_edit_price)
    if not row:
        raise HTTPException(status_code=404, detail="Сотрудник не найден")
    return {"ok": True, **dict(row)}
...
async def _render_sms_notification(kind: str, lang: str, order_num: str, count: int) -> str:
    tmpl = await db.get_config(f"sms_{kind}_template_{lang}") or await db.get_config(f"sms_{kind}_template_ru") or ""
    if not tmpl:
        return ""
...
async def _send_sms_notification(kind: str, order_id: int, phone: str, order_num: str, count: int, lang: str,
    ...
    """kind: 'pickup' (при заборе) | 'ready' (при готовности). lang выбирается ЯВНО тем, кто
    отправляет (водитель/менеджер/оператор знает язык клиента) — автоопределения нет.
    Каждая попытка (успех/провал) пишется в order_activity — вкладка «Уведомления» карточки
    заказа читает эти же записи, отдельной таблицы не понадобилось (см. 2026-07-30).
    Возвращает (отправлено?, причина_если_нет) — нужно кнопке «📲 SMS» показать явную ошибку,
    а не молча ничего не делать."""
    lang = lang if lang in ("ru", "uz") else "ru"
    if (await db.get_config(f"sms_{kind}_enabled") or "") != "true":
        return False, "Отправка SMS отключена (тогл выключен в Тексты SMS)"
    if not phone:
        return False, "У заказа нет телефона клиента"
    text = await _render_sms_notification(kind, lang, order_num, count)
    if not text:
        return False, "Шаблон SMS не настроен"
    try:
        ok = await send_sms(phone, text)
    except Exception as e:
        logging.warning(f"{kind} SMS failed for {phone}: {e}")
        ok = False
...
class ManualSmsRequest(BaseModel):
    ...
    kind: str = "pickup"      # "pickup" | "ready" — какой шаблон/тогл использовать
...
async def manual_send_sms(order_id: int, req: ManualSmsRequest, staff=Depends(require_perm("orders"))):
    """Кнопка «📲 SMS» во вкладке «Уведомления» карточки заказа — тот же шаблон и та же
    логика, что при автоматической отправке (_send_sms_notification), но вручную и с явным
    выбором языка (для случаев: не отправилось само, нужно повторить). Уважает тогл
    «Включить SMS при заборе/Готово» — если выключен, явно возвращает ok:false с причиной,
    а не молча делает вид что отправлено (см. запрос пользователя 2026-07-30)."""
    order = await db.get_order_by_id(order_id)
    if not order: raise HTTPException(404, "Заказ не найден")
...
async def driver_route_take_pickup(order_id: int, req: PickupTakeRequest, staff=Depends(get_current_staff)):
    """«Забрал» на маршруте типа «Забор» — confirmed/new → pickup, создаёт пустые
    ...
    иначе точка маршрута не отметится выполненной в списке."""
    if not _can_drive(staff): raise HTTPException(403, "Нет доступа")
    order = await db.get_order_by_id(order_id)
    if not order: raise HTTPException(404, "Заказ не найден")
    if order.get("status") not in ("new", "confirmed"):
...
async def driver_route_undo_pickup(order_id: int, staff=Depends(get_current_staff)):
    ...
    удаляет созданные пустые позиции, возвращает точку маршрута в pending (зеркалит
    bot.py: action == "undo" для cur == "pickup")."""
    if not _can_drive(staff): raise HTTPException(403, "Нет доступа")
    order = await db.get_order_by_id(order_id)
    if not order: raise HTTPException(404, "Заказ не найден")
    if order.get("status") != "pickup":
...
SITE_SETTINGS_DEFAULTS = {
    ...
    # SMS клиенту, когда заказ готов к выдаче/доставке — тот же формат/лимит.
    "sms_ready_enabled":       "false",
    "sms_ready_template_ru":   "ARTEZ: zakaz {order_num} gotov. Voprosy: {phones} bot {bot_link} Status na sayte {site_link}",
    "sms_ready_template_uz":   "ARTEZ: {order_num} buyurtma tayyor. Savollar: {phones} bot {bot_link} Holat: {site_link}",
    # ОСАГО партнёр
    ...
    # Каналы водителей/доставщиков (маршруты)
...
async def public_site_video(lang: str, request: Request):
...
async def autodial_import_active(cid: int, _=Depends(_get_admin)):
    """Импортирует дозвонившихся (answered_at не пусто) из кампании в единую базу
    актуальных контактов (active_contacts)."""
